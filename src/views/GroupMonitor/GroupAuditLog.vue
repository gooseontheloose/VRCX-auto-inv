        ...
        <template v-else>
            ...
            <div class="space-y-3">
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <Select :model-value="String(auditPageSizeDisplay)" @update:model-value="(v) => (auditPageSizeDisplay = Number(v))">
                            <SelectTrigger class="w-24 h-8 text-sm"><SelectValue /></SelectTrigger>
                            <SelectContent>
                        ...
                        <Button variant="outline" size="sm" class="h-8" @click="exportCsv(filteredAuditLogs, 'audit-log.csv')">
                    ...
                    <div class="rounded-lg border overflow-hidden">
                        <table class="w-full text-sm">
                            <thead class="bg-muted/40 border-b">
                                <tr>
                                    <th class="text-left px-3 py-2 text-xs font-medium text-muted-foreground cursor-pointer hover:text-foreground" @click="toggleSort(sortAuditCol, sortAuditDir, 'created_at')">
                                        <span class="flex items-center gap-1">Time <ChevronsUpDown class="size-3 opacity-40" /></span>
                                    </th>
                                    <th class="text-left px-3 py-2 text-xs font-medium text-muted-foreground">Event</th>
                                    <th class="text-left px-3 py-2 text-xs font-medium text-muted-foreground cursor-pointer hover:text-foreground" @click="toggleSort(sortAuditCol, sortAuditDir, 'actorDisplayName')">
                                        <span class="flex items-center gap-1">Actor <ChevronsUpDown class="size-3 opacity-40" /></span>
                                    </th>
                                    <th class="text-left px-3 py-2 text-xs font-medium text-muted-foreground cursor-pointer hover:text-foreground" @click="toggleSort(sortAuditCol, sortAuditDir, 'targetDisplayName')">
                                        <span class="flex items-center gap-1">Target <ChevronsUpDown class="size-3 opacity-40" /></span>
                                    </th>
                                    <th class="text-left px-3 py-2 text-xs font-medium text-muted-foreground">Description</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-if="isLoadingAudit"><td colspan="5" class="text-center py-10 text-muted-foreground"><RefreshCw class="size-4 animate-spin inline mr-2" />Loading…</td></tr>
                                <tr v-else-if="filteredAuditLogs.length === 0"><td colspan="5" class="text-center py-10 text-muted-foreground text-sm">No entries found for the selected filters.</td></tr>
                                <template v-else>
                                    <tr v-for="entry in pagedAuditLogs" :key="entry.id" class="border-b last:border-0 hover:bg-muted/30 transition-colors">
                                        <td class="px-3 py-2 text-xs tabular-nums text-muted-foreground whitespace-nowrap">{{ fmtDate(entry.created_at) }}</td>
                                        <td class="px-3 py-2"><Badge variant="outline" :class="['text-xs whitespace-nowrap', auditBadgeClass(entry.eventType)]">{{ auditLabel(entry.eventType) }}</Badge></td>
                                        <td class="px-3 py-2 text-sm">
                                            <button v-if="entry.actorId" class="font-medium hover:underline cursor-pointer" @click="openProfileById(entry.actorId)">
                                                {{ resolveAuditActor(entry) || '…' }}
                                            </button>
                                            <span v-else-if="resolveAuditActor(entry)" class="font-medium">{{ resolveAuditActor(entry) }}</span>
                                            <span v-else class="text-muted-foreground">—</span>
                                        </td>
                                        <td class="px-3 py-2 text-sm">
                                            <button v-if="entry.targetId" class="hover:underline cursor-pointer" @click="openProfileById(entry.targetId)">
                                                {{ resolveAuditTarget(entry) || '…' }}
                                            </button>
                                            <span v-else-if="resolveAuditTarget(entry)" class="text-muted-foreground">{{ resolveAuditTarget(entry) }}</span>
                                            <span v-else class="text-muted-foreground">—</span>
                                        </td>
                                        <td class="px-3 py-2 text-xs text-muted-foreground max-w-xs truncate">{{ entry.description || '—' }}</td>
                                    </tr>
                                </template>
                            </tbody>
                        </table>
                    ...
                    <!-- pagination controls -->
                    <div class="flex items-center justify-between py-2 px-1 flex-wrap gap-2">
                        <div class="text-xs text-muted-foreground">
                            <span>{{ filteredAuditLogs.length }} entries</span>
                            <span v-if="auditAutoLoading" class="ml-2 text-muted-foreground/60">(loading more…)</span>
                            <span v-else-if="auditLimitReached && !auditFullyLoaded" class="ml-2 text-amber-500">
                                · VRChat API hit 7700 entry offset limit — fetching older history by date
                            </span>
                            <span v-else-if="auditFullyLoaded" class="ml-2 text-green-600">· fully loaded</span>
                        </div>
                        <div class="flex items-center gap-1">
                            <Button variant="outline" size="sm" class="h-7 px-2 text-xs" :disabled="auditDisplayPage === 0" @click="auditDisplayPage--">← Prev</Button>
                            <span class="text-xs tabular-nums px-2">{{ auditDisplayPage + 1 }} / {{ auditTotalPages }}</span>
                            <Button variant="outline" size="sm" class="h-7 px-2 text-xs" :disabled="auditDisplayPage >= auditTotalPages - 1" @click="auditDisplayPage++">Next →</Button>
                        </div>
                    </div>
            </div>
        </template>
